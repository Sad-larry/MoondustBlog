        ...
        <el-dialog :visible.sync="addOrEdit" width="40%" top="3vh">
            ...
            <el-form label-width="80px" size="medium" :model="article">
                ...
                <el-form-item label="文章标签">
                    <el-tag
                        v-for="(item, index) of article.tagListVo"
                        :key="index"
                        style="margin: 0 1rem 0 0"
                        :closable="true"
                    ...
                    <el-popover
                        ...
                        width="460"
                        trigger="click"
                        v-if="article.tagListVo.length < 3"
                    >
...
export default {
    ...
    methods: {
        ...
        uploadCover(response) {
            this.article.thumbnail = response.data;
        },
        beforeUpload(file) {
        ...
        saveOrUpdateArticle() {
            ...
            if (this.article.tagListVo.length == 0) {
                this.$message.error("文章标签不能为空");
                return false;
            }
            if (this.article.thumbnail.trim() == "") {
        ...
        addTag(item) {
            if (this.article.tagListVo.indexOf(item.tagName) == -1) {
                this.article.tagListVo.push(item.tagName);
            }
        },
        removeTag(item) {
            const index = this.article.tagListVo.indexOf(item);
            this.article.tagListVo.splice(index, 1);
        },
    },
    computed: {
        tagClass() {
            return function (item) {
                const index = this.article.tagListVo.indexOf(item.tagName);
