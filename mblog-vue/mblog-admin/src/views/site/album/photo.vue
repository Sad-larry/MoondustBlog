    ...
    <el-card class="main-card">
      ...
      <div class="album-info">
        ...
        <div class="album-detail">
          ...
          <div>
            <span v-if="albumInfo.info" class="album-desc">{{ albumInfo.info }}</span>
            <el-button
        ...
        <div class="operation">
          ...
          <el-button
            ...
            v-hasPermi="['system:photo:movePhoto']"
          >移动到</el-button>
          <el-button
            type="danger"
            @click="deletePhoto"
            size="small"
            icon="el-icon-deleteItem"
            v-hasPermi="['system:photo:delete']"
          >删除</el-button>
        </div>
      </div>
      <!-- 照片列表 -->
                ...
                <div class="photo-opreation">
                  <el-dropdown @command="handleCommand">
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item :command="'update' + item.id" v-hasPermi="['system:photo:update']">
                        <i class="el-icon-edit" />编辑
                      </el-dropdown-item>
                      <el-dropdown-item :command="'delete' + item.id" v-hasPermi="['system:photo:delete']">
                        <i class="el-icon-delete" />删除
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </div>
                <el-image fit="cover" class="photo-img" :src="item.url" :preview-photoSrc-list="photoList" />
...
export default {
  ...
  methods: {
    ...
    updatePhotoAlbum() {
      ...
      movePhoto({ albumId: this.albumId, ids: ids })
        .then((response) => {
    ...
    deletePhoto(id) {
      ...
      this.$confirm("此操作将把页面删除, 是否继续?", "提示", {
        ...
        .then(() => {
          delPhoto(this.selectPhotoIdList)
            .then((response) => {
