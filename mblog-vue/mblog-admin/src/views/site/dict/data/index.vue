                ...
                <el-table-column label="是否默认" width="100" align="center">
                    <template slot-scope="scope">
                        <span v-for="item in yesOrNoList" :key="item.value">
                            <el-tag :type="item.style" v-if="scope.row.isDefault == item.value">{{ item.label }}
                            </el-tag>
                        </span>
                    </template>
                </el-table-column>
            ...
            <el-form :model="form" :rules="rules" ref="form">
                ...
                <el-form-item label="字典类型" :label-width="formLabelWidth" prop="dictId">
                    <el-select disabled v-model="form.dictId" placeholder="请选择字典类型" style="width:200px">
                        <el-option v-for="item in dictTypeList" :key="item.id" :label="item.type" :value="item.id">
                        </el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="回显样式" :label-width="formLabelWidth">
                    <el-select v-model="form.style" clearable placeholder="回显样式" style="width:140px" >
                        <el-option v-for="item in styleType" :key="item.key" :label="item.label" :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item>
    ...
    methods: {
        ...
        handleDelete(row) {
            ...
            this.$confirm("此操作将把字典数据删除, 是否继续?", "提示", {
                ...
                delDictData(row.id).then(response => {
