  ...
  <div class="app-container">
    ...
    <el-form v-show="showSearch" :inline="true" ref="form" :model="queryParams" label-width="68px">
      <el-form-item label="字典名称">
        <el-select size="small" v-model="queryParams.dictId" placeholder="请选择字典名称" @change="handleFind">
          <el-option v-for="item in dictTypeList" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          v-hasPermi="['system:dictData:delete']"
        >删除</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="listDictData"></right-toolbar>
    </el-row>

    <div style="margin-top: 5px">
      <el-table border :data="dictDataList" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="是否默认" width="100" align="center">
          <template slot-scope="scope">
            <span v-for="item in yesOrNoList" :key="item.value">
              <el-tag :type="item.style" v-if="scope.row.isDefault == item.value">{{ item.label }}</el-tag>
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="发布状态" width="100" align="center">
          <template slot-scope="scope">
            <span v-for="item in isPublishList" :key="item.value">
              <el-tag :type="item.style" v-if="scope.row.isPublish == item.value">{{ item.label }}</el-tag>
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" min-width="200">
          <template slot-scope="scope">
            <el-button
              ...
              type="primary"
              size="mini"
              v-hasPermi="['system:dictData:update']"
            >编辑</el-button>
            <el-button
    ...
    <el-dialog center :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form">
        ...
        <el-form-item label="字典类型" :label-width="formLabelWidth" prop="dictId">
          <el-select disabled v-model="form.dictId" placeholder="请选择字典类型" style="width:200px">
            <el-option v-for="item in dictTypeList" :key="item.id" :label="item.type" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="回显样式" :label-width="formLabelWidth">
          <el-select v-model="form.style" clearable placeholder="回显样式" style="width:140px">
            <el-option v-for="item in styleType" :key="item.key" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="系统默认" :label-width="formLabelWidth" prop="isDefault">
          <el-radio-group v-model="form.isDefault" size="small">
            <el-radio v-for="item in yesOrNoList" :key="item.value" :label="item.value" border>
              {{
              item.label
              }}
            </el-radio>
...
export default {
  ...
  created() {
    //传递过来的dictId
    this.queryParams.dictId = this.$route.query.dictId;
