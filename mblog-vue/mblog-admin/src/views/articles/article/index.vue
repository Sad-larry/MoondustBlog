<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :row-class-name="tableRowIsPublish"
    >
      ...
      <el-table-column label="类型" align="center" prop="type">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.isOriginal === 0" type="warning">{{ isOriginalList[0] }}</el-tag>
          <el-tag v-else-if="scope.row.isOriginal === 1" type="success">{{ isOriginalList[1] }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="标签" align="center" prop="tagNames" width="220">
        <template slot-scope="scope">
          <el-tag
            ...
            align="center"
            type="primary"
            v-for="(item, index) in strSplit(scope.row.tagNames)"
            :key="index"
          >{{ item }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="置顶" align="center" prop="isStick">
        <template slot-scope="scope">
          <el-switch
      ...
      <el-table-column label="操作" align="center" width="300">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:article:pubOrShelf']"
          >下架</el-button>
          <el-button
...
export default {
  ...
  methods: {
    ...
    /** 标识未发布的博客文章 */
    tableRowIsPublish({ row, rowIndex }) {
      if (row.isPublish === 0) {
