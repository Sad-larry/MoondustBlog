    ...
    <div class="app-container">
        <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px"
            ...
            <el-form-item label="任务名称" prop="jobName">
                <el-input v-model="queryParams.jobName" placeholder="请输入任务名称" clearable @clear="resetQuery"
                    @keyup.enter.native="handleQuery" />
            </el-form-item>
            <el-form-item label="任务组名" prop="jobGroup">
                <el-select @change="handleQuery" v-model="queryParams.jobGroup" placeholder="请选择任务组名" clearable
                    size="small">
                    <el-option v-for="dict in jobDictList" :key="dict.value" :label="dict.label" :value="dict.value" />
                </el-select>
            </el-form-item>
            <el-form-item label="任务状态" prop="status">
                <el-select @change="handleQuery" v-model="queryParams.status" placeholder="请选择任务状态" clearable
                    size="small">
                    <el-option v-for="dict in jobStatusList" :key="dict.value" :label="dict.label"
                        :value="dict.value" />
                </el-select>
            </el-form-item>
            <el-form-item>
        ...
        <el-row :gutter="10" class="mb8">
            ...
            <el-col :span="1.5">
                <el-button type="info" plain icon="el-icon-s-operation" size="mini" :disabled="single"
                    @click="handleJobLog">日志</el-button>
            </el-col>
            <el-col :span="1.5">
            ...
            <el-table-column label="任务组名" align="center" prop="jobGroup">
                <template slot-scope="scope">
                    <template v-for="(dict, index) in jobDictList">
                        <el-tag v-if="dict.value === scope.row.jobGroup" :key="index" :type="dict.style">
                            {{ dict.label }}
                        </el-tag>
                    </template>
                </template>
        ...
        <pagination v-show="total > 0" :total="total" :page.sync="queryParams.pageNum"
            :limit.sync="queryParams.pageSize" @size-change="getList" />
                    ...
                    <el-col :span="12">
                        <el-form-item label="任务分组" prop="jobGroup">
                            <el-select v-model="form.jobGroup" placeholder="请选择">
                                <el-option v-for="dict in jobDictList" :key="dict.value" :label="dict.label"
                    ...
                    <el-col :span="24">
                        <el-form-item label="错误策略" prop="misfirePolicy">
                            <el-radio-group v-model="form.misfirePolicy" size="small">
                                <el-radio-button v-for="item in jobMisfireList" :key="item.value" :label="item.value">
                    ...
                    <el-col :span="12">
                        <el-form-item label="状态">
                            <el-radio-group v-model="form.status">
                                <el-radio v-for="dict in jobStatusList" :key="dict.value" :label="parseInt(dict.value)">
                                    {{ dict.label }}</el-radio>
                            </el-radio-group>
                        </el-form-item>
                    </el-col>
                ...
                <el-row>
                    ...
                    <el-col :span="12">
                        <template v-for="(item, index) in jobDictList">
                            <el-form-item v-if="item.value === form.jobGroup" :key="index" label="任务分组：">
                                {{ item.label }}
                            </el-form-item>
                        </template>
